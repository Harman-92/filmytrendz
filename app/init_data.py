...
for i in range(2):
	...
	with app.app_context():
		...
		query = "insert into movie (url, imdb_id, tmdb_id, director, title," \
				"description, genre, external_rating, rating, year, popularity, original_language, country, actors) values " \
		...
		for r in range(1, sheet.nrows):
			...
			rating = float(sheet.cell(r, 13).value)

			values = (url, imdb_id, tmdb_id, director, title,
					  description, genre, external_rating, rating, year, popularity, original_language, country, actors)
		...
		print(sheet.nrows, ' movies are inserted into database\n')
